...
class Doctor:
    ...
    def _check_installation(self):
        ...
        if python_version >= (3, 8):
            self._add_success(table, f"Python {python_version.major}.{python_version.minor}.{python_version.micro}")
        else:
            self._add_error(table, f"Python {python_version.major}.{python_version.minor} (requires 3.8+)")
        ...
        if self.config:
            try:
                ...
                if hasattr(self.config.publishers, 'jira') and self.config.publishers.jira:
                    if (hasattr(self.config.publishers.jira, 'enabled') and self.config.publishers.jira.enabled):
    ...
    def _check_environment(self):
        ...
        required_packages = [
            "yaml", "requests", "click", "pydantic", 
            "structlog", "rich", "packaging"
        ]
    ...
    def _check_permissions(self):
        ...
        if self.config:
            try:
                if hasattr(self.config.publishers, 'clickup') and hasattr(self.config.publishers.clickup, 'cache'):
